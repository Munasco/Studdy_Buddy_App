...
def room(request, pk):
    ...
    room_messages = room.messages_in_room.all().order_by('-created')
...
def userProfile(request, pk):
    ...
    rooms_messages = user.users_messages.all()
    topics = Topic.objects.all()
    context = {'user': user, 'rooms': rooms,
               'room_messages': rooms_messages, 'topics': topics}
...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        Room.objects.create(host=request.user, topic=topic, name=request.POST.get(
            'name'), description=request.POST.get('description'))
        return redirect('home')
    context = {'form': form, "topics": topics, "action_type": "Create"}
